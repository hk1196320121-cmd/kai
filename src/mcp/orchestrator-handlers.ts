...
export function registerOrchestratorHandlers(server: McpServer, db: KaiDB): void {
  ...
  server.tool("kai_task_execute", TaskExecuteSchema, async ({ task_id }) => {
    ...
    return textContent({ success: result.success, task_id, agent: "hermes", error: result.error });
  });
  ...
  server.tool("kai_idea_pause", IdeaPauseSchema, async ({ idea_id }) => {
    ...
    return textContent({ paused_tasks: result.paused, cancelled_cron_jobs: result.cancelled });
  });
  ...
  server.tool("kai_execution_status", ExecutionStatusSchema, async ({ idea_id, task_id, feedback }) => {
    ...
    return textContent({
      tasks: tasks.map((t) => ({
        id: t.id, title: t.title, status: t.status, type: t.type, retry_count: t.retry_count,
      })),
      results: results.map((r) => ({
        id: r.id, success: Boolean(r.success), duration_ms: r.duration_ms, completed_at: r.completed_at,
      })),
