...
import { dim, header, kv, table } from "../format";
...
export function renderChampion(champion: PromptChampion): string {
  ...
  lines.push(kv("variant", champion.variant_id.slice(0, 8)));
  ...
  lines.push(kv("promoted", champion.promoted_at.slice(0, 10)));

  const lockStatus = champion.is_locked ? `Yes ${dim("[LOCKED]")}` : "No";
...
export function renderGeneList(genes: PromptGene[]): string {
  ...
  const rows = genes.map((gene) => [
    ...
    gene.task,
    gene.type,
    gene.content.length > 50 ? `${gene.content.slice(0, 47)}...` : gene.content,
  ]);
...
export function renderTournamentResults(results: PromptTournament[]): string {
  ...
  const rows = results.map((t) => {
    ...
    const confidence =
      t.judge_confidence != null ? t.judge_confidence.toFixed(2) : "N/A";
