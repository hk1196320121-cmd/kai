...
export function registerAutopilotCommands(program: Command): void {
  const autopilot = program
    .command("autopilot")
    .description("Autopilot status and management");
  ...
  autopilot
    ...
    .action((opts) => {
      ...
      if (sessions.length === 0) {
        ...
        console.log(
          "Sessions are created automatically when Claude Code starts with Kai hooks installed.",
        );
        return;
      }
      ...
      if (activeSession) {
        console.log(
          `Active: ${activeSession.session_id} (started ${activeSession.started_at})`,
        );
        console.log();
      }
      ...
      for (const s of sessions) {
        ...
        const duration = s.stopped_at
          ...
          : " (active)";
        console.log(
          `  ${status} ${s.session_id} — ${s.observations_count} obs, ${s.traits_derived} traits${duration}`,
        );
      }
    });
}
